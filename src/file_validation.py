...
class FileValidation:
    ...
    def validate_and_process_files(self):
        ...
        for file_pattern in self.type_files:
            list_files = glob.glob(os.path.join(data_raw_directory, file_pattern))
            ...
            for file_path in list_files:
                ...
                validation_file_path = os.path.join(data_validation_directory, file_name)
                if os.path.exists(validation_file_path):
                    base_name, ext = os.path.splitext(file_name)
                    count = self.count_names.get(base_name, 1)
                    new_name = f'{base_name}_{count}{ext}'

                    while os.path.exists(os.path.join(data_validation_directory, new_name)):
                        count += 1
                        new_name = f'{base_name}_{count}{ext}'

                    self.count_names[base_name] = count + 1
                    new_file_path = os.path.join(data_validation_directory, new_name)
                    shutil.move(file_path, new_file_path)
                    print(f'O arquivo {file_name} foi renomeado para {new_name} e movido para o diretório de validação.')
                else:
                    shutil.move(file_path, validation_file_path)
                    print(f'O arquivo {file_name} foi movido para o diretório de validação.')
